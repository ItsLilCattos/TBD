...
#get ffmpeg or lavalink, I prefer lava for u cause u know java, yes I prefer for me cause it is easier
#make sure to use youtube_dl as well, also people switch to reactjs for web
...
#regular dumb errros we pass
class YTDLError(Exception):
    pass

#I used cogs so all of it is in class, no prolly not
class YTDLSource(discord.PCMVolumeTransformer):
    ...
    @classmethod
    async def search_source(cls, ctx: commands.Context, bots: commands.bot, search: str, *, loop: asyncio.BaseEventLoop = None):
        channel = ctx.channel
        loop = loop or asyncio.get_event_loop()
        ...
        partial = functools.partial(cls.ytdl.extract_info, cls.search_query, download=False)
        ...
        try:
            m = await bots.wait_for('message', check = check, timeout=10)
...
class Music(commands.Cog):
    ...
    async def _volume(self, ctx: commands.Context, *, volume: int):
        ...
        if volume not in range(1,100):
         await ctx.send('Volume must be between 1 and 100')

         ctx.voice_state.volume = volume / 100
        
        if ctx.voice_state.is_playing():
            ctx.voice_state.current.voice.volume = volume / 100
    ...
    async def _loopq(self, ctx: commands.Context, _queue):
    ...
    async def _search(self, ctx: commands.Context, *, search: str):
        ...
        async with ctx.typing():
            try:
                source = await YTDLSource.search_source(ctx, self.bot, search, loop=self.bot.loop)
            except YTDLError as e:
                await ctx.send('An error occurred while processing this request: {}'.format(str(e)))
            else:
...
client = commands.Bot(command_prefix='|', case_insensitive=True, description="The Superior Bot")
client.add_cog(Music(client))
def setup(client):
    client.add_cog(Music(client))
    print("Music cog is loading")
